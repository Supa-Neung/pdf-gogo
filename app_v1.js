...
async function addPageNumbers(){
  ...
  pages.forEach((p,i)=>{const n=start+i;let text=fmt==="number"?`${n}`:fmt==="thai-total"?` ${n} / ${start+pages.length-1}`:`Page ${n} / ${start+pages.length-1}`;const size=10,tw=font.widthOfTextAtSize(text,size),m=18;let x=pos.endsWith("l")?m:pos.endsWith("r")?p.getWidth()-tw-m:(p.getWidth()-tw)/2;let y=pos.startsWith("t")?p.getHeight()-size-m:m;p.drawText(text,{x,y,size,font,color:rgb(.25,.25,.25)});});
...
async function deletePagePDF(){
    ...
    await setResult(
        result,
        `${safeName(state.file.name)}_delete-page.pdf`
    );
}
...
async function reorderPDF(){
    ...
    await setResult(
        result,
        `${safeName(state.file.name)}_reorder.pdf`
    );
}

async function duplicatePagePDF(){
    if(!requireFile())
        throw new Error("กรุณาเลือก PDF หลัก");
    const pdfDoc =
        await PDFDocument.load(
    ...
    if(!Number.isInteger(pageNoInput) || pageNoInput<1 || pageNoInput>total)
        throw new Error(`หมายเลขหน้าต้องอยู่ระหว่าง 1 ถึง ${total}`);
    const pageNo = pageNoInput - 1;
    const copiedPage =
    ...
    await setResult(
        result,
        `${safeName(state.file.name)}_duplicate-page.pdf`
    );
}
